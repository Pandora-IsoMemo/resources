...
test_that("object of class fruits is created", {
  ...
  obsvn <- (default = matrix(c(-19, 12, -17, 11, -21, 14, -20, 9, -19, 10),
                  ...
                  dimnames = list(paste0("ind", 1:5),
                                  valueNames$targets)))
  obsvnError <- (default = matrix(rep(0.5, 10), ncol = length(valueNames$targets),
                                  dimnames = list(paste0("ind", 1:5),
                                                  valueNames$targets)))
  ...
  concentration <- list(matrix(c(80, 52, 10, 90, 92, 20, 48, 90, 10, 8),
                          ...
                          ncol = length(valueNames$fractions),
                          dimnames = list(valueNames$sources,
                                          valueNames$fractions)))
  concentrationUncert <- list(matrix(c(0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5),
                                ...
                                dimnames = list(valueNames$sources,
                                                valueNames$fractions)))
  source <- list(list(Carbon = matrix(c(-19, -21, -25, -13, -19, -24, -26, -29, -16, -23),
                        nrow = length(valueNames$sources),
                        ncol = length(valueNames$fractions),
                        dimnames = list(valueNames$sources,
                                        valueNames$fractions)),
                 Nitrogen = matrix(c(5, 7, 1, 10, 11, 0, 0, 0, 0, 0),
                        ...
                        dimnames = list(valueNames$sources,
                                        valueNames$fractions))))
  sourceUncert <- list(list(Carbon = matrix(rep(0.5, 10),
                        nrow = length(valueNames$sources),
                        ncol = length(valueNames$fractions),
                        dimnames = list(valueNames$sources,
                                        valueNames$fractions)),
  ...
  modelOptions <- list(modelType = "1", modelWeights = TRUE, modelConcentrations = TRUE,
  ...
  x <- fruits(data,
              modelOptions,
              valueNames)
  expect_named(x, c("data", "priors", "modelOptions", "constants",
