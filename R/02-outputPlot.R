outputPlotUI <- function(id) {
  ...
  fluidRow(
    ...
    sidebarPanel(
      width = 3,
      selectInput(inputId = ns("estType"),
                  label = "Select group of estimates",
                  choices = c("Sources", "User Estimates")),
      selectInput(inputId = ns("groupType"),
                  label = "Display by:",
                  choices = c("Estimate" = "Estimate", "Target" = "Target")),
      selectInput(inputId = ns("filterType"),
                  label = "Select category:",
                  choices = NULL),
      selectInput(inputId = ns("individuals"),
      ...
      selectInput(inputId = ns("plotType"),
                  ...
                  choices = c("BoxPlot", "KernelDensity",
                              "Histogram")),
      checkboxInput(inputId = ns("showLegend"), 
                    label = "Show legend",
                    value = FALSE),
      conditionalPanel(
        condition = "input.plotType == 'Histogram'",
        ns = ns,
        sliderInput(inputId = ns("histBins"), 
      ...
      selectInput(inputId = ns("contributionLimit"), label = "Limit contribution axis",
...
outputPlot <- function(input, output, session, model, values) {
  ...
  observe({
    ...
    observeEvent(input$estType, {
      if(grepl(paste(c("Source contributions",
                      "Component contributions",
                      "Source contributions by proxy"), collapse="|"),
               input$estType)){
  ...
  observe({
    ...
    if(!is.null(model()) && model()$fruitsObj$modelOptions$modelType != "1"){
      if(model()$fruitsObj$modelOptions$hierarchical){
        filterTypeChoices <-  c(filterTypeChoices,
  ...
  observe({
    updatePickerInput(session = session, inputId = "groupVars",
                      choices = groupChoices(), selected = groupChoices())
    
  })
  
  addRow <- function(df) {
    rbind(df, data.frame(index = nrow(df) + 1, y = NA, group = NA,
