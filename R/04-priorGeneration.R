getNimblePrior <- function(priorParameters, prior, replacements, n, individualNames,
  ...
  if (grepl("=", nimblePrior) == FALSE){
    if(applyUnc == FALSE){
      return(paste0("prior", n[2], "_", individualNames[n[1]],
  ...
  if (grepl("=", nimblePrior) == TRUE){
    ...
    return(c(paste0("prior", n[2], "_", paste0(individualNames[n[1]], c("a", "b")),
                    " ~ dconstraint(", nimblePrior, c("*1/2", "*-1/2"), ")"),
             paste0("pUnc_",n[2], " ~ dunif(0,",Unc,")")))
    } else {
      nimblePrior <- c(paste0(gsub("=", "<", nimblePrior)), paste0(gsub("=", ">", nimblePrior)))
      return(paste0("prior", n[2], "_", paste0(individualNames[n[1]],
                                               c("a", "b")), " ~ dconstraint(", nimblePrior, c("*1/2", "*-1/2"), ")"))
    }
  }
}
...
getUserEstimatesExpressions <- function(userEstimatesParameters,
  ...
  for (i in 1:length(userEstimatesParameters)){
    userEstimates <- gsub(paste0(" ", userEstimatesParameters[i], " "),
...
getNamesInsideBrackets <- function(priorString){
  priorString <- gsub("(^[^\\[]*\\[)|\\]((?<=\\])[^\\]]*$)",
                      "", priorString, perl = TRUE) # rm start & end
  ...
  unique(unlist(lapply(1:length(priors), function(x){
    ...
    if(length(na.omit(priorDistributions)) > 0){
      priorDist <- unlist(lapply(1:length(priorDistributions), function(xx){
        dP <- unlist(regmatches(priorDistributions[xx],
    ...
    valueNames$valProxies <- apply(expand.grid(valueNames$sources,
                                               valueNames$fractions,
                                               valueNames$targets),
                                   1, paste, collapse = "-")
    valueNames$contrProxies <- apply(expand.grid(valueNames$targets,
                                                 valueNames$sources),
                                     1, paste, collapse = "-")
    valueNames$weightValues <- apply(expand.grid(valueNames$targets,
                                                 valueNames$fractions),
                                     1, paste, collapse = "-")
    valueNames$concValues <- apply(expand.grid(valueNames$sources,
                                               valueNames$fractions),
                                   1, paste, collapse = "-")
    ...
    priorList <- lapply(1:nTargetsAdjusted, function(j){
      replacements <- list(valProxies = tryCatch({sapply(1:length(valProxiesParameters),
                                                         function(x) {
                                                           if (modelOptions$modelType %in% c("2", "3")) {
                                                             paste0("I_[",match(strsplit(valProxiesParameters,
                                                                                         "-")[[x]][1],
                                                                                valueNames$sources), ",",
                                                                    match(strsplit(valProxiesParameters,
                                                                                   "-")[[x]][2],
                                                                          valueNames$fractions), ",",
                                                                    match(strsplit(valProxiesParameters,
                                                                                   "-")[[x]][3],
                           ...
                           contrProxies = tryCatch({ if (modelOptions$modelType != "1") {
                             sapply(1:length(contrProxiesParameters),
                                    function(x) {paste0("theta_[", j, ",",
                                                        match(strsplit(contrProxiesParameters,
                                                                       "-")[[x]][1],
                                                              valueNames$targets), ",",
                                                        match(strsplit(contrProxiesParameters,
                                                                       "-")[[x]][2],
                                                              valueNames$sources), "]")})
                           } else { sapply(1:length(contrProxiesParameters),
                                           function(x) {paste0("theta_[",
                                                               match(strsplit(contrProxiesParameters,
                                                                              "-")[[x]][1],
                                                                     valueNames$targets), ",",
                                                               match(strsplit(contrProxiesParameters,
                           ...
                           weightValues = tryCatch({sapply(1:length(weightValuesParameters),
                                                           function(x) {
                                                             if (!(modelOptions$modelType %in% c("4", "5"))) {
                                                             paste0("W_[", match(strsplit(weightValuesParameters,
                                                                                              "-")[[x]][1],
                                                                                     valueNames$targets), ",",
                                                                               match(strsplit(weightValuesParameters,
                                                                                              "-")[[x]][2],
                                                                                     valueNames$fractions), "]")
                                                               } else {
                                                                 paste0("W_[", j, ",", match(strsplit(weightValuesParameters,
                                                                                              "-")[[x]][1],
                                                                                     valueNames$targets), ",",
                                                                        match(strsplit(weightValuesParameters,
                                                                                       "-")[[x]][2],
                                                   ...
                                                   error = function(cond) return(c())),
                           concValues = tryCatch({sapply(1:length(concValuesParameters),
                                                         function(x) {
                                                           if (!(modelOptions$modelType %in% c("3", "4", "5"))) {
                                                           paste0("C_[",match(strsplit(concValuesParameters,
                                                                                            "-")[[x]][1],
                                                                                   valueNames$sources), ",",
                                                                             match(strsplit(concValuesParameters,
                                                                                            "-")[[x]][2],
                                                                                   valueNames$fractions), "]")
                                                           } else {
                                                             paste0("C_[", j, ",", match(strsplit(concValuesParameters,
                                                                                         "-")[[x]][1],
                                                                                valueNames$sources), ",",
                                                                    match(strsplit(concValuesParameters,
                                                                                   "-")[[x]][2],
                                                 ...
                                                 error = function(cond) return(c())),
                           consValues = tryCatch({sapply(1:length(consValuesParameters),
                                                         function(x) {paste0("mu[", j, ",",
                                                                             match(strsplit(consValuesParameters,
                                                                                            "-")[[x]][2],
                                                                                   valueNames$targets), "]")})},
                                                 error = function(cond) return(c())),
                           sources = tryCatch({if (modelOptions$modelType != "1") {paste0("alpha_[", j, ",",
                                                                                          match(sourceParameters,
                                                                                                valueNames$sources), "]")}
                             else{
                               paste0("alpha_[",
                                      match(sourceParameters,
                             ...
                             error = function(cond) return(c())),
                           fractions = tryCatch({if (modelOptions$modelType != "1") {paste0("beta_[", j, ",",
                                                                                            match(fractionParameters,
                                                                                                  valueNames$fractions), "]")}
                             else{
                               paste0("beta_[",
                                      match(fractionParameters,
                           ...
                           hierValues = tryCatch({getHierPriors(hierValuesParameters = 
                                                                  hierValuesParameters,
                                                                type = "alpha_[",
                                                                covariates = covariates,
                                                                names1 = valueNames$sources)},
                                                 error = function(cond) return(c())),
                           hierValuesBeta = tryCatch({getHierPriors(hierValuesParameters = 
                                                                  hierValuesParametersBeta,
                                                                  type = "beta_[",
                                                                covariates = covariates,
                                                                names1 = valueNames$fractions)},
                                                 error = function(cond) return(c())),
                           hierValuesTheta = tryCatch({getHierPriors(hierValuesParameters = 
      ...
      replacements2 <- try({sapply(1:length(vMatch), function(i){replacements[[vMatch[i]]][mIndex[i]]})}, silent = TRUE)
...
getAllMainInteractions <- function(covariates, Names1, Names2 = NULL, vars = NULL) {
  ...
  if (ncol(covariates) > 1) {
    ...
    for (i in 1:ncol(selectionsL1)) {
      interactionsL1 <- c(interactionsL1,
  ...
  if (ncol(covariates) > 2) {
    ...
    for (i in 1:ncol(selectionsL2)) {
      interactionsL2 <- c(interactionsL2,
...
getHierPriors <- function(hierValuesParameters, covariates, names1, type = "alpha_[", names2 = NULL){
  sapply(1:length(hierValuesParameters), function(x){
    ...
    sourceSplits <- match(strsplit(hierValuesParameters, "-")[[x]][length(strsplit(hierValuesParameters, "-")[[x]])],
                          names1)
    hierarchicalReturn <- paste0(type, which(sapply(1:nrow(covariates),
                                                         function(y) all(hiersplits %in% covariates[y, ]))), ",",sourceSplits, "]")
    if(length(hierarchicalReturn) > 1){
      hierarchicalReturn <- paste0("(", paste0(hierarchicalReturn, collapse = "+"), ")/", length(hierarchicalReturn))
    } 
    } else {
      hiersplits <- strsplit(hierValuesParameters, "-")[[x]][-c(length(strsplit(hierValuesParameters, "-")[[x]]),
                                                                length(strsplit(hierValuesParameters, "-")[[x]]) - 1)]
      n2Splits <- match(strsplit(hierValuesParameters, "-")[[x]][length(strsplit(hierValuesParameters, "-")[[x]]) - 1],
                            names2)
      n1Splits <- match(strsplit(hierValuesParameters, "-")[[x]][length(strsplit(hierValuesParameters, "-")[[x]])],
                        names1)
