plotExport <- function(input, output, session, plotFun, type = "plot", plotly = FALSE){
  observeEvent(input$export, {
    ...
    plotOutputElement <- if (plotly) plotlyOutput(session$ns("plotly"))
    else plotOutput(session$ns("plot"), height = "300px")

    exportTypeChoices <- if (plotly) c("png", "jpeg", "svg", "pdf")
    else c("png", "pdf", "svg", "tiff")
