...
produceOutText <- function(fruitsObj, diagnostic){
  outText <- paste0("ReSources ran ", fruitsObj$modelOptions$nchains,
  ...
  diagFailIndividual <- (((1 - pnorm(abs(diagnostic[grepl("alpha", names(diagnostic))]))) * 2) %>% 
                           p.adjust("BH") < 0.05)
  if(fruitsObj$modelOptions$modelType != "1"){
    failedIndividuals <- unique(rep(1:fruitsObj$constants$nTargets,
                                    fruitsObj$constants$nSources)[which(diagFailIndividual)])
  } else {
  ...
  if(any(diagFailIndividual) == FALSE){
    ...
    outText <-  paste0(outText, "<br> Convergence failures found for sources, for target(s) no. ",
