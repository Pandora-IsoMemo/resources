OxCalOutputUI <- function(id) {
  ...
  tagList(
    ...
    fluidRow(
      column(
        width = 3,
        selectInput(ns("aquaticCurve1"), label = "Aquatic Curve 1", choices = NULL)
      ),
      column(width = 3, selectInput(
        ns("OxCalA"), "Estimate",
        choices = c("none")
      )),
      column(width = 3, numericInput(ns("meanDeltaR1"), "Mean Delta R 1", 0)),
      column(width = 3, numericInput(ns("sdDeltaR1"), "SD Delta R 1", 1))
    ),
    fluidRow(
      column(
        width = 3,
        selectInput(ns("aquaticCurve2"), label = "Aquatic Curve 2", choices = NULL)
      ),
      column(width = 3, selectInput(
        ns("OxCalB"), "Estimate 2",
        choices = c("none")
      )),
      column(width = 3, numericInput(ns("meanDeltaR2"), "Mean Delta R 2", 0)),
      column(width = 3, numericInput(ns("sdDeltaR2"), "SD Delta R 2", 1))
    ),
    radioButtons(
      ns("bins"),
      "Type of estimate",
...
OxCalOutput <-
  function(input,
    ...
    terrestrialCurves <- reactive({
      file <-
        "https://pandoradata.earth/dataset/46fe7fc7-55a4-493d-91e8-c9abffbabcca/resource/afaf48cf-1a72-4996-89ba-44815bc84d8e/download/oxcal_terrestrial_curve.txt"
      parseCurveFile(readLines(file, warn = FALSE))
    })

    aquaticCurves1 <- reactive({
      file <-
        "https://pandoradata.earth/dataset/46fe7fc7-55a4-493d-91e8-c9abffbabcca/resource/9626d93b-013d-4b02-b3d4-e4091f6ed600/download/oxcal_aquatic_curve_1.txt"
      parseCurveFile(readLines(file, warn = FALSE))
    })

    aquaticCurves2 <- reactive({
      file <-
        "https://pandoradata.earth/dataset/46fe7fc7-55a4-493d-91e8-c9abffbabcca/resource/e5a197c8-15c7-4752-a2d4-15ad1c39ca19/download/oxcal_aquatic_curve_2.txt"
      parseCurveFile(readLines(file, warn = FALSE))
    })
    ...
    observe({
      updateSelectInput(session,
        "terrestrialCurve",
        choices = getCurveTitles(terrestrialCurves())
      )
      ...
      withProgress(
        {
          TextOxCal <- createOxCalText(
            ...
            terrestrialCurve = terrestrialCurves()[as.numeric(input$terrestrialCurve)],
...
createOxCalText <- function(model,
  ...
  oxcalText <- lapply(basicCode, function(part) {
    if (part == "%%Terrestrial_curve_VAR1%%") {
      return(getCurveFormula(terrestrialCurve))
    }
    if (part == "%%Aquatic_curve_1_VAR1%%") {
      return(getCurveFormula(aquaticCurve1,
        mean = meanDeltaR1,
        sd = sdDeltaR1
      ))
    }
    if (part == "%%Aquatic_curve_2_VAR1%%") {
      return(getCurveFormula(aquaticCurve2,
        mean = meanDeltaR2,
        sd = sdDeltaR2
      ))
    }
