...
getResultStatistics <- function(parameters, userEstimates, fruitsObj,
  ...
  statisticsFunctions <- c(transformStatNames(c("mean", "sd", "lower 95%", "upper 95%")),
                           statisticsFunctions)
  if(bins){
    statisticsNames <- c(statisticsNames, paste("bin", seq(0, 1, by = 0.01), sep = "_"))
    statisticsFunctions <- c(statisticsFunctions,
  ...
  if(NROW(userEstimates) > 0){
    colnames(userEstimates) <- unlist(lapply(colnames(userEstimates),
  ...
  if(agg == FALSE){
    resultMatrix <- do.call("rbind", lapply(1:length(renamedChains), function(x){
      if(NCOL(renamedChains[[x]]) == 0){
        ...
        resultMatrix <- data.frame(Parameter_Type = names(renamedChains[x]),
        ...
        if(fruitsObj$modelOptions$modelType != "1" & 
           (names(renamedChains))[x] %in% c("Source contributions", "Component contributions",
          ...
          repInd <- rep(rep(1:length(rownames(fruitsObj$data$obsvn)),
          ...
          if(NROW(fruitsObj$data$covariates) > 0 & NCOL(fruitsObj$data$covariates) > 0 & 
            ...
            if(NCOL(fruitsObj$data$covariates) > 1) {
              resultMatrix$covariateInteraction <- interaction(split(t(fruitsObj$data$covariates),
    ...
    resultMatrix <- do.call("rbind", lapply(1:length(renamedChains), function(x){
      ...
      if(fruitsObj$modelOptions$modelType != "1" & 
        ...
        if(NROW(fruitsObj$data$covariates) > 0 & NCOL(fruitsObj$data$covariates) > 0 & 
          ...
          covariateVectors <- lapply(1:ncol(fruitsObj$data$covariates),
                                     function(x) rep(fruitsObj$data$covariates[, x],
                                                     repTimes))
          names(covariateVectors) <- colnames(fruitsObj$data$covariates)
          
          if(NCOL(fruitsObj$data$covariates) > 1) {
            covariateVectors$interactions <- rep(interaction(split(t(fruitsObj$data$covariates),
                                                                   colnames(fruitsObj$data$covariates)),
                                                             drop = FALSE), repTimes)
          }
        ...
        resultMatrix <- data.frame(Target = rep(rownames(fruitsObj$data$obsvn), repTimes),
        ...
        resultMatrix <- data.frame(resultMatrix, Target = rep("all", nrow(resultMatrix)),
      ...
      if(nrow(resultMatrix) > 0){
        ...
        if(exists("resultMatrix2") && nrow(resultMatrix2) > 0){
          resultMatrix2 <- data.frame(Parameter_Type = (names(renamedChains))[x], resultMatrix2,
...
transformUIStatNames <- function(statistics){
  ...
  if(!is.null(statistics)){
    ...
    if(statistics[7] == TRUE){
      statisticsReturn <- c(statisticsReturn,
                            paste0("function(y) min(2 * mean(y > ", statistics[8],
...
translateParameters <- function(parameters, fruitsObj, addNameTypes = FALSE){
  ...
  if(fruitsObj$modelOptions$modelType != "1"){
    sourceNameCombinations <- expand.grid(rownames(fruitsObj$data$obsvn),
                                          fruitsObj$valueNames$sources, stringsAsFactors = FALSE)
  ...
  targetNameCombinations <- expand.grid(rownames(fruitsObj$data$obsvn),
                                        fruitsObj$valueNames$targets, stringsAsFactors = FALSE)
  ...
  weightsNameCombinations <- expand.grid(fruitsObj$valueNames$targets,
                                         fruitsObj$valueNames$fractions, stringsAsFactors = FALSE)
  if(ncol(weights) > 1){
  ...
  translatedParameters <- list("Source contributions" = sources,
  ...
  if(addNameTypes){
    translatedParameters <- lapply(1:length(translatedParameters), function(x){
      if(NCOL(translatedParameters[[x]]) > 1){
        colnames(translatedParameters[[x]]) <- paste0(names(translatedParameters)[x],
...
applyNames <- function(nameCombinations){
  unlist(lapply(1:nrow(nameCombinations),
                function(x) paste0("[",
