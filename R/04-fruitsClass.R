...
fruits <- function(data,
  ...
  if (modelOptions$modelWeights == FALSE | all(is.na(data$weights))){
    ...
    data$weightsUncert <- matrix(0, ncol = length(valueNames$targets),
                                 nrow =  length(valueNames$targets),
                                 dimnames = list(valueNames$targets,
                                                 valueNames$targets))
  }
  ...
  data <- data[sapply(1:(length(names(data)) - 1),
                      function(x) grepl(names(data)[-which(names(data) == "covariates")][x],
...
checkValueNames <- function(valueNames) {
  ...
  checkNames(valueNames,
             ...
             argName = "valueNames")
  invisible(mapply(FUN = checkClass,
                   x = valueNames,
...
checkModelOptions <- function(modelOptions) {
  ...
  checkNames(modelOptions,
             ...
             namesExpected = c("modelType", "modelWeights", "modelConcentrations",
...
checkClass <- function(x,
  ...
  if (!(typeActual %in% classesExpected)) {
    stop(paste0(if (!is.null(argName)) paste0(argName, " has "),
  ...
  if(length(measurementNames) > length(unique(measurementNames))){
      ...
      if(sum(x == measurementNames) > 1){
        ...
        sds <- sqrt(matrix(colSums((sds[x == measurementNames, , drop = FALSE] ^ 2) / 
                                (sds[x == measurementNames, , drop = FALSE] ^ 2)) / 
                        colSums(1 / sds[x == measurementNames, , drop = FALSE] ^ 2) + 
                          sapply(1:length(meansCombined), function(y) weighted.var(x = means[x == measurementNames, , drop = FALSE][,y],
                                              w = sds[x == measurementNames, , drop = FALSE][,y])),
                      nrow = 1,
                      dimnames = list(x, colnames(means))))
        return(list(means = signif(meansCombined, 5), sds = signif(sds,5)))
      } else {
      return(list(means = round(means[x == measurementNames,],5),
      ...
      if(sum(x == measurementNames) > 1){
        ...
        meansCombined <- abind::abind(lapply(1:dim(means)[3], function(k) {
          matrix(colSums(means[x == measurementNames, , k, drop = FALSE] / 
                                ...
                                nrow = 1,
                                dimnames = list(x, colnames(means)))}), along = 3)
          
        sds <- abind::abind(lapply(1:dim(means)[3], function(k) {
          sqrt(matrix(colSums((sds[x == measurementNames, ,k, drop = FALSE] ^ 2) / 
                                     (sds[x == measurementNames, , k, drop = FALSE] ^ 2)) / 
                             colSums(1 / sds[x == measurementNames, , k , drop = FALSE] ^ 2) + 
                             sapply(1:dim(meansCombined)[2], function(y) weighted.var(x = means[x == measurementNames, , k , drop = FALSE][,y,],
                                                                                      w = sds[x == measurementNames, , k , drop = FALSE][,y,])),
                           nrow = 1,
                           dimnames = list(x, colnames(means))))}), along = 3)
        return(list(means = signif(meansCombined, 5), sds = signif(sds,5)))
      } else {
        return(list(means = round(means[x == measurementNames, , ,drop =F],5),
...
transformRepeatedMeasures <- function(data, modelOptions){
  ...
  data2$source <- combineSources(data$source, data$sourceUncert, type = "means",
                                 perm = !modelOptions$modelWeights, modelType = modelOptions$modelType)
  data2$sourceUncert <- combineSources(data$source, data$sourceUncert, type = "sds",
                                       perm = !modelOptions$modelWeights, modelType = modelOptions$modelType)
  
  if(modelOptions$includeSourceOffset){
    data2$sourceOffset <- combineSources(data$sourceOffset, data$sourceOffsetUnc, type = "means",
                                         perm = !modelOptions$modelWeights, modelType = modelOptions$modelType)
    data2$sourceOffsetUnc <- combineSources(data$sourceOffset, data$sourceOffsetUnc, type = "sds",
                                            perm = !modelOptions$modelWeights, modelType = modelOptions$modelType)
  }
  if(!is.null(unlist(data2$sourceT1))){
    data2$sourceT1 <- combineSources(data$sourceT1, data$sourceUncertT1, type = "means",
                                     perm = !modelOptions$modelWeights, modelType = modelOptions$modelType)
    data2$sourceUncertT1 <- combineSources(data$sourceT1, data$sourceUncertT1, type = "sds",
                                           perm = !modelOptions$modelWeights, modelType = modelOptions$modelType)
  }
  if(!is.null(unlist(data2$sourceT2))){
    data2$sourceT2 <- combineSources(data$sourceT2, data$sourceUncertT2, type = "means",
                                     perm = !modelOptions$modelWeights, modelType = modelOptions$modelType)
    data2$sourceUncertT2 <- combineSources(data$sourceT2, data$sourceUncertT2, type = "sds",
                                           perm = !modelOptions$modelWeights, modelType = modelOptions$modelType)
  }
  if(!is.null(unlist(data2$sourceT3))){
    data2$sourceT3 <- combineSources(data$sourceT3, data$sourceUncertT3, type = "means",
                                     perm = !modelOptions$modelWeights, modelType = modelOptions$modelType)
    data2$sourceUncertT3 <- combineSources(data$sourceT3, data$sourceUncertT3, type = "sds",
                                           perm = !modelOptions$modelWeights, modelType = modelOptions$modelType)
  }
  return(data2)
}
...
replaceSourceTerms <- function(sourceTerm, sourceUncertTerm, modelOptions, valueNames, constant = FALSE){
  ...
  if(!is.null(sourceTerm) && 
     !(all(sapply(1:length(sourceTerm),
                  function(y) all(sapply(sourceTerm[[y]], function(x) all(is.na(x))))))) && 
     !(all(sapply(1:length(sourceTerm),
                  function(y) all(sapply(sourceTerm[[y]], function(x) all(is.na(x)))))))){
    allColumnNames  <- lapply(1:length(sourceTerm),
                              function(j) unlist(lapply(1:length(sourceTerm[[j]]),
    ...
    sourceTerm[[i]][] <- lapply(1:length(sourceTerm[[i]]),
                              function(x){
                                if(!modelOptions$modelWeights){
                                  x <- 1
                                }
                                if(any(!(allColumnNames %in% colnames(sourceTerm[[i]][[x]])))){
                                  ...
                                  newTerm <- cbind(sourceTerm[[i]][[x]], matrix(0, ncol = length(newCol),
    ...
    sourceUncertTerm[[i]][] <- lapply(1:length(sourceUncertTerm[[i]]),
                                function(x){
                                  if(!modelOptions$modelWeights){
                                    x <- 1
                                  }
                                  if(any(!(allColumnNames %in% colnames(sourceUncertTerm[[i]][[x]])))){
                                    ...
                                    newTerm <- cbind(sourceUncertTerm[[i]][[x]], matrix(0, ncol = length(newCol),
...
replaceOtherTerms <- function(targetTerm, targetUncertTerm, modelOptions, constant = FALSE){
  ...
  if(!is.null(targetTerm) && !(all(sapply(targetTerm, function(x) all(is.na(x)))) && 
    ...
    targetUncertTerm <- replace(targetUncertTerm,is.na(targetUncertTerm) | constant == TRUE,
                                modelOptions$minUnc)
  } else {
    targetTerm <- NULL
...
replaceCombineCovSource <- function(CovList, modelOptions, valueNames, source = TRUE){
  if(!is.null(CovList) && 
     !(all(sapply(1:length(CovList), function(z) all(sapply(CovList[[z]],
                  function(x){ all(sapply(x, function(y){ all(is.na(y))}))})))))){
    ...
    #CovList[[i]] <- lapply(1:length(CovList[[i]]), function(x){ 
      ...
      if(!modelOptions$modelWeights & source == TRUE & ncol(CovList[[i]]) > 1){
        ...
        m[(length(valueNames$fractions) + 1) : (2*length(valueNames$fractions)),
          (length(valueNames$fractions) + 1) : (2*length(valueNames$fractions))] <- CovList[[i]]
        CovList[[i]] <- m
      }
      #CovList[[i]]
      #y
...
replaceValues <- function(data, modelOptions, valueNames){
  ...
  weights <- replaceOtherTerms(data$weights, data$weightsUncert, modelOptions,
                                modelOptions$weightsDist == "constant")
  data$weights <- weights[[1]]
  data$weightsUncert <- weights[[2]]

  #Ersetze Concentration-Werte
  concentrations <- lapply(1:length(data$concentration),
                           function(x){ replaceOtherTerms(data$concentration[[x]],
  ...
  data$concentrationCov <- replaceCombineCovSource(data$concentrationCov,
                                           modelOptions, valueNames, source = FALSE)
  
  #Ersetze Source-Term-Werte
  T0 <- replaceSourceTerms(data$source, data$sourceUncert, modelOptions, valueNames, 
  ...
  T1 <- replaceSourceTerms(data$sourceT1, data$sourceUncertT1, modelOptions, valueNames, 
  ...
  T2 <- replaceSourceTerms(data$sourceT2, data$sourceUncertT2, modelOptions, valueNames, 
  ...
  T3 <- replaceSourceTerms(data$sourceT3, data$sourceUncertT3, modelOptions, valueNames, 
  ...
  T0 <- replaceOtherTerms(data$obsvn, data$obsvnError, modelOptions,
...
repCov <- function(means, unc){
  ...
  res <- Reduce(`+`,Map(`*`, lapply(1:length(covMatrices),
