...
compileRunModel <- function(fruitsObj, progress = FALSE, onlySim = FALSE,
  ...
  model <- try({nimbleModel(code = fruitsObj$modelCode,
                      name = 'FRUITS',
  ...
  if (length(fruitsObj$userEstimates[[1]]) > 0){
    ...
    userEstParameters <- lapply(1:length(fruitsObj$userEstimates[[1]]),
                                function(x) gsub(" ", "", paste0(strsplit(
                                 fruitsObj$userEstimates[[1]][[x]], "=")[[1]][1], "_",
                                    rownames(fruitsObj$data$obsvn)))) %>% unlist %>%
      na.omit
    } else {
    userEstParameters <- lapply(1:length(fruitsObj$userEstimates[[1]]),
                                function(x) gsub(" ", "", paste0(strsplit(
                                  fruitsObj$userEstimates[[1]][[x]], "=")[[1]][1], "_All"))) %>% unlist %>% na.omit
    }
  ...
  conf <- configureMCMC(model, monitors2 = userEstParameters,
  ...
  samples <- runMCMC(FRUITSMCMC, niter = fruitsObj$modelOptions$iterations +
  ...
  userEstimateSamples <- normalizeUserEstimates(userEstimateSamples,
  ...
  return(list(parameters = parameters, userEstimateSamples = userEstimateSamples, wAIC = wAIC,
...
normalizeUserEstimates <- function(userEstimateSamples, userEstimatesGroups, indNames){
  ...
  if(NROW(userEstimateSamples) > 0 & length(userEstimatesGroups) > 0){
    userEstimatesGroups <- userEstimatesGroups[sapply(1:length(userEstimatesGroups),
                                                      function(x) !is.null(userEstimatesGroups[[x]]$name) && 
    ...
    userEstimateSamples <- lapply(1:length(groupNames), function(x){
      ...
      if(userEstimatesGroups[[x]]$normalize == TRUE & !is.null(indNames)){
        ...
        estNew <- do.call("cbind", lapply(splits,
                                          function(x) return(est[,x] / rowSums(est[,x]))))
...
computePPValues <- function(parameters, dataMeans, dataSds){
  densitiesLog <- lapply(1:nrow(dataMeans), function(y){
    ...
    dObs <- sapply(1:ncol(dataMeans), function(x) dnorm(yobs[,x], parameters[1:nrow(parameters), paste0("mu[",y,", ", x,"]")], dataSds[y,x])) %>% log %>% rowSums()
    dModel <- sapply(1:ncol(dataMeans), function(x) dnorm(yModel[,x], parameters[1:nrow(parameters), paste0("mu[",y,", ", x,"]")], dataSds[y,x])) %>% log %>% rowSums()
    list(dObs, dModel)
  })
  
  densTotalObs <- Reduce("+",lapply(densitiesLog, function(x){x[[1]]}))
  densTotalModel <- Reduce("+",lapply(densitiesLog, function(x){x[[2]]}))
...
bugfixFraction1 <- function(data, constant) {
  ...
  data$source <- abind(data$source, along = 2,
                       array(0, dim = c(
                         constant$nSources, 1, constant$nProxies
                       )))
  
  data$sourceUncert <- abind(data$sourceUncert, along = 2,
                             array(0, dim = c(
                               constant$nSources, 1, constant$nProxies
                             )))
  if (!is.null(data$sourceOffset)) {
  data$sourceOffset <- abind(data$sourceOffset, along = 2,
                       array(0, dim = c(
                         constant$nSources, 1, constant$nProxies
                       )))
  
  data$sourceOffsetUnc <- abind(data$sourceOffsetUnc, along = 2,
                             array(0, dim = c(
                               constant$nSources, 1, constant$nProxies
                             )))
  }
  if (!is.null(data$sourceT1)) {
    data$sourceT1 <- abind(data$sourceT1, along = 2,
                           array(0, dim = c(
                             constant$nSources, 1, constant$nProxies
                           )))
    
    data$sourceUncertT1 <- abind(data$sourceUncertT1, along = 2,
                                 array(0, dim = c(
                                   constant$nSources, 1, constant$nProxies
                                 )))
  }
  if (!is.null(data$sourceT2)) {
    data$sourceT2 <- abind(data$sourceT2, along = 2,
                           array(0, dim = c(
                             constant$nSources, 1, constant$nProxies
                           )))
    
    data$sourceUncertT2 <- abind(data$sourceUncertT2, along = 2,
                                 array(0, dim = c(
                                   constant$nSources, 1, constant$nProxies
                                 )))
  }
  if (!is.null(data$sourceT3)) {
    data$sourceT3 <- abind(data$sourceT3, along = 2,
                           array(0, dim = c(
                             constant$nSources, 1, constant$nProxies
                           )))
    
    data$sourceUncertT3 <- abind(data$sourceUncertT3, along = 2,
                                 array(0, dim = c(
                                   constant$nSources, 1, constant$nProxies
                                 )))
  }
