plotTargets <- function(fruitsObj, modelResults, individual, estType = "Source contributions",
  ...
  if(contributionLimit == "0-100%"){
    ...
    if(nrow(pointDat) > 0){ pointDat <- pointDat %>% mutate_(y = ~ y * 100) }
  }
  ...
  if(returnType == "data"){
    ...
    if(!is.null(binSize)){
      sequence <- seq(floor(100 * min(modelResults$estimate)) / 100,
                      floor(100 * (max(modelResults$estimate) + binSize)) / 100, by = binSize)
      
      modelResults$bin <- cut(modelResults$estimate,
                              breaks = sequence, include.lowest = TRUE)
      
      modelResults <- modelResults %>% group_by_(~group, ~bin) %>% summarise(nSamples = n()) %>% ungroup
    }
    return(modelResults)
  }
  ...
  if (plotType == "BoxPlot"){
    ...
    if(nrow(pointDat) > 0){
      ...
      p <- p + geom_point(data = pointDat, mapping = aes_(x = ~ group, y = ~ y),
  ...
  if (plotType == "KernelDensity"){
    ...
    p <- ggplot(modelResults, aes_(x = ~ estimate, fill = ~ group)) + geom_density(alpha = 0.3) +
      ylab(ylabel) + xlab(xlabel) 
    if(contributionLimit == "0-100%"){
      p <- p + xlim(c(0, 100)) 
    }
  ...
  if (plotType == "Trace"){
    ...
    modelResults$X <- rep(1:(nrow(modelResults) / length(unique(modelResults$group))),
  ...
  if (plotType == "AutoCor"){
    ...
    acfDataNew$X <- rep(1:(nrow(acfDataNew) / length(acfData)),
                        length(acfData))
  ...
  p <- p + theme(axis.title.x = element_text(size = xTextSize),
